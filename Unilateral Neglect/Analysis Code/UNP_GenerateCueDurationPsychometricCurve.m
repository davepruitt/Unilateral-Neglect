function [ psycho_curve, hit_matrix, sem_psycho_curve ] = UNP_GenerateCueDurationPsychometricCurve ( session_list, method )
...
psychometric_cue_durations = [250 100 50 10 5 2 1];

if (method == 0)
    hit_matrix = zeros(length(psychometric_cue_durations), 2);
else
    hit_matrix = zeros(length(psychometric_cue_durations), 2, length(session_list));
end
...
for s=1:length(session_list)
    ...
    for i = 1:length(non_aborted_trials)
        row_index = find(psychometric_cue_durations == non_aborted_trials(i).cue_duration, 1, 'first');
