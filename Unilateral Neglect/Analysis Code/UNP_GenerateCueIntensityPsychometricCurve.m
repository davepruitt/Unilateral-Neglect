function [ psycho_curve, hit_matrix, sem_psycho_curve ] = UNP_GenerateCueIntensityPsychometricCurve ( session_list, method )
...
possible_cue_intensities = [0 1 2 4 8 16 32 64 128 255];

if (method == 0)
    hit_matrix = zeros(length(possible_cue_intensities), 2);
else
    hit_matrix = zeros(length(possible_cue_intensities), 2, length(session_list));
end
...
for s=1:length(session_list)
    ...
    for i = 1:length(non_aborted_trials)
        row_index = find(possible_cue_intensities == non_aborted_trials(i).cue_intensity, 1, 'first');
