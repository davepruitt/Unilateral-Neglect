...
nosepoke_history = zeros(1, NOSEPOKE_BUFFER_SIZE);
...
while (ishandle(hObject) && (h.run ~= SESSION_IS_STOPPED))
    ...
    elseif (~isempty(trial_nosepoke_history) && length(trial_nosepoke_history) > SAMPLES_PER_SECOND)
        ...
        plot(h.current_trial_axis, [hit_start hit_start], [0 1200], 'LineStyle', '--', 'LineWidth', 3, 'Color', [0 0.7 0]);
        plot(h.current_trial_axis, [hit_end hit_end], [0 1200], 'LineStyle', '--', 'LineWidth', 3, 'Color', [1 0 0]);
    end
    ...
    if (h.trial_state == TRIAL_STATE_NOT_BEGUN)
        ...
        if (rat_has_entered_nosepoke && (h.run ~= SESSION_IS_PAUSED))
            ...
            trial_nosepoke_history = zeros(ceil(SAMPLES_PER_SECOND * 2 + (actual_pre_trial_wait + h.response_time_threshold) * SAMPLES_PER_SECOND), 1);
